    ...
    public function place_order($data)
    {
        ...
        if (!empty($product_variant)) {
            ...
            for ($i = 0; $i < count($product_variant); $i++) {
                ...
                // $p_id = $product_id[$i]['product_id'];
                 $p_id = $product_variant[$i]['product_id'];//product_id
                    $this->db->where('id', $p_id);
                    $this->db->where('seller_id', $seller_id);
                    $query_cat_id  = $this->db->get('products')->result_array(); 
                 
                
                
                $category_id = $query_cat_id[0]['category_id'];
                ...
                $this->db->where('p_id1', $p_id)->or_where('p_id2', $p_id)->or_where('p_id3', $p_id);
                $this->db->group_end(); // End the group of OR conditions
                $query_comm  = $this->db->get('promation_product')->result_array();
                // print_r($this->db->last_query());
                //  print_r($query_comm);
                // die();
              $is_Campain_product = false;
                if (!empty($query_comm)) {
                    $admin_com = $query_comm[0]['commission']*$subtotal[$i]/100;
                    $seller_com = $subtotal[$i] -$admin_com ;
                    
                    $is_Campain_product = true;
                    
                    $campain_history_data= [
                    ...
                    'campain_id' => $query_comm[0]['commission'],
                    'admin_commission'=>$query_comm[0]['commission'],
                    'admin_amount'=> $admin_com,
                    'seller_amount'=> $seller_com,
                  
                ];
                //campain_order
                 $this->db->insert('campain_order', $campain_history_data);
                    ...
                    $query_comm2  = $this->db->get('seller_data')->result_array();
                   // print_r($query_comm2[0]['commission']);
                    
                    if (!empty($query_comm2)) {
                        $admin_com = $query_comm2[0]['commission']*$subtotal[$i]/100;
                        //echo("---->");
                        //echo($admin_com);
                    }
                    else{
                ...
                $product_variant_data[$i] = [
                    ...
                    'price' => $pv_price[$i],
                    'tax_percent' => $tax_percentage[$i],
                    'admin_commission_amount'=> $admin_com,
                    'seller_commission_amount'=> $seller_com,
                    'tax_amount' => $tax_amount[$i],
                    'sub_total' => $subtotal[$i],
                    'status' =>  json_encode(array(array($status, date("d-m-Y h:i:sa")))),
                    'active_status' => $status,
                    'otp' => ($system_settings['is_delivery_boy_otp_setting_on'] == '1') ? $otp : 0,
                    'is_Campain_product' => ($is_Campain_product == false) ? 0 : 1,
                ];
            ...
            if (trim(strtolower($data['payment_method'])) != 'paypal' || trim(strtolower($data['payment_method'])) != 'stripe') {
                ...
                if (!empty($user_fcm_id)) {
                    ...
                    $data = send_notification($fcmMsg, $user_fcm_id);
                //     print_r($data);
                // die();
                }
                ...
                //seller notification
// print_r($seller_id);
                    $user_fcm = fetch_details('users', ['id' => $seller_id], 'fcm_id');
                ...
                $data_seller = send_notification($fcmMsg, $user_fcm_id);
        ...
        // die();
                ...
                $data = insert_details($admin_notifi, 'system_notification');
                // print_r($data);
                // die();
    ...
    public function get_orders_list(
        ...
        //  echo "<pre>", htmlspecialchars(print_r($product_count)), "</pre>";
        // print_r($count_res);
        // die();
        ...
        $search_res = $this->db->select(' o.* , u.username, db.username as delivery_boy,oi.admin_commission_amount, oi.seller_commission_amount')
            ->join(' `users` u', 'u.id= o.user_id', 'left')
            ->join(' `order_items` oi', 'oi.order_id= o.id', 'left')
            ->join('product_variants v ', ' oi.product_variant_id = v.id', 'left')
            ->join('products p ', ' p.id = v.product_id ', 'left')
        ...
        $i = 0;
        //  echo "<pre>", htmlspecialchars(print_r($user_details)), "</pre>";
        // print_r($count_res);
        // die();
        ...
        foreach ($user_details as $row) {
            ...
            if (!empty($row['items'])) {
                ...
                $tempRow['admin_commission_amount'] = $row['admin_commission_amount'];
                $tempRow['seller_commission_amount'] = $row['seller_commission_amount'];
